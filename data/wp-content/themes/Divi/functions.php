...
/**
 * Margick Fix
 * Set divi to default build
 */
class DBCOMEnableBuilderByDefault {
		
	function init() {
		add_action('load-post-new.php', array($this, 'enableDiviBuilderByDefault'));
		add_action('load-post.php', array($this, 'enableDiviBuilderByDefaultWithEditPost'));
		add_action('admin_bar_menu', array($this, 'removeDiviBuilderButton'), 9999);
	}

	public function removeDiviBuilderButton() {
		global $wp_admin_bar;
		// $wp_admin_bar->remove_menu('et-disable-visual-builder');
		$wp_admin_bar->remove_menu('et-use-visual-builder');
	}

	function enableDiviBuilderByDefaultWithEditPost() {
		$post_id = filter_input(INPUT_GET, 'post', FILTER_VALIDATE_INT);
		if ($post_id) {
			$post = get_post($post_id);
			$this->convertToVisualBuilderPostEdit($post_id, $post);
		}
	}

	public function convertToVisualBuilderPostEdit($post_id, $post) {
