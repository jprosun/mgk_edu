...
class URE_View
{
    ...
    protected function get_no_need_capabilities()
    {
        return ["create_users", "activate_plugins", "delete_plugins", "edit_plugins", "install_plugins", "update_plugins"];
    }

    protected function is_no_need_capabilities($cap_id, $no_need_capabilities)
    {
        $is_no_need_cap = false;
        foreach ($no_need_capabilities as $no_need_cap) {
            if ($cap_id == $no_need_cap) {
                $is_no_need_cap = true;
                continue;
            }
        }

        return $is_no_need_cap;
    }
    ...
    public function _show_capabilities($for_role = true, $edit_mode = true)
    {
        ...
        $no_need_capabilities = $this->get_no_need_capabilities();
        ...
        foreach ($full_capabilities as $capability) {
            ...
            if ($this->is_no_need_capabilities($cap_id, $no_need_capabilities)) {
                continue;
            }

            if (!$user_is_ure_admin) {
                if (
                    isset($ure_caps[$cap_id]) ||
            ...
            if ($for_role) {
                if (
                    isset($roles[$current_role]['capabilities'][$cap_id]) &&
    ...
    public function advertise_pro()
    {
        ...
        <div id="ure_pro_advertisement" style="clear:left;display:block; float: left;">
            <a href="https://www.role-editor.com?utm_source=UserRoleEditor&utm_medium=banner&utm_campaign=Plugins "
                target="_new">
                <?php
                ...
                <label for="ure_hide_pro_banner">
                    <input type="checkbox" name="ure_hide_pro_banner" id="ure_hide_pro_banner"
                        onclick="ure_main.hide_pro_banner();" />&nbsp;Thanks, hide this banner.
                </label>
                <?php
                }
                ?>
    ...
    {
        ?>
        <div id="ure_task_status"
            style="display:none;position:absolute;top:10px;right:10px;padding:10px;background-color:#000000;color:#ffffff;">
            <img src="<?php echo URE_PLUGIN_URL . 'images/ajax-loader.gif'; ?>" width="16" height="16" />
            <?php esc_html_e('Working...', 'user-role-editor'); ?>
        </div>
        <?php
    }
    ...
    public function display_caps($for_role = true, $edit_mode = true)
    {
        ...
        <table id="ure_caps_container" cellpadding="0" cellspacing="0">
            <tr>
                <td id="ure_caps_groups_title"><span style="font-weight: bold;">
                        <?php esc_html_e('Group', 'user-role-editor'); ?>
                    </span> (
                    <?php esc_html_e('Total', 'user-role-editor'); ?>/
                    <?php esc_html_e('Granted', 'user-role-editor'); ?>)
                </td>
                    ...
                    <div class="ure-table">
                        ...
                        if ($this->show_select_all()) {
                            ...
                            <div class="ure-table-cell">
                                <input type="checkbox" id="ure_select_all_caps" name="ure_select_all_caps"
                                    value="ure_select_all_caps" />
                            </div>
                            <?php
                        }
                        ?>
                        <div class="ure-table-cell ure-caps-option nowrap">
                            ...
                            <input type="text" id="quick_filter" name="quick_filter" value="" size="10"
                                onkeyup="ure_main.filter_capabilities(this.value);" />&nbsp;&nbsp;&nbsp;
                            <input type="checkbox" id="granted_only" name="granted_only" />
                            <label for="granted_only">
                                <?php esc_html_e('Granted Only', 'user-role-editor'); ?>
                            </label>&nbsp;
                        </div>
                        <div class="ure-table-cell ure-caps-option nowrap">
                            <?php esc_html_e('Columns:', 'user-role-editor'); ?>
                            <select id="caps_columns_quant" name="caps_columns_quant"
                                onchange="ure_main.change_caps_columns_quant();">
