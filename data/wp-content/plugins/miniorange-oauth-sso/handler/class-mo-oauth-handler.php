...
class MO_OAuth_Handler {
	...
	* @param object $value
	* 
	* @return [boolean] true if is not empty, false otherwise
	*/
   public function is_not_null($value) {
	...
	/**
	 * sanitize and unslash text
	 * 
	 * @param string $text
	 * 
	 * @return [string]
	 */
	public function sanitize_unslash_text($text) {
	 ...
	 * @return [string]
	 */
	public function get_current_app_name() {
		$currentappname = '';
		$tappname       = isset($_COOKIE['tappname']) ? $_COOKIE['tappname'] : "";
		$appname        = isset($_SESSION['appname']) ? $_SESSION['appname'] : "";
		$state          = isset($_SESSION['state']) ? $_SESSION['state'] : "";
		MOOAuth_Debug::mo_oauth_log('appname: '.$appname.'. state: '.$state);

		if ($this->is_not_null($tappname)) {
			$currentappname = $this->sanitize_unslash_text($tappname);
		} elseif ($this->is_not_null($appname)) {
			$currentappname = sanitize_text_field($appname);
		} elseif ($this->is_not_null($state)) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Ignoring nonce verification because we are fetching data from URL and not on form submission.
			$currentappname = $this->sanitize_unslash_text(base64_decode($state)); //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Recommended, WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- Base64 encoding will be required to fetch current app name from state. Sanitizing late for $_REQUEST['state'] as we need to sanitize after decode.
		}
	 ...
	 * Get app by name
	 */
	public function get_default_app_name() {
		$appname = '';
		$applist = get_option('mo_oauth_apps_list');
		if ( $applist ) {
	 ...
	 * Get app by name
	 */
	public function get_app_by_name($appname) {
		$returnapp = false;
		$applist = get_option('mo_oauth_apps_list');
		if ( $applist ) {
			foreach ($applist as $key => $app) {
	...
	public function get_current_app() {
		$appname = $this->get_current_app_name();
		return $this->get_app_by_name($appname);
	}
