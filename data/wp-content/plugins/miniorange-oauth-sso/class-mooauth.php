 ...
 *
 * @package    oauth
 * @author     Margick <admin@margick.com>
 * @license    MIT/Expat
 * @link       https://margick.com/
 */
...
class MOOAuth
{
	...
	public function __construct()
	{
		...
		$this->register_idp_oauth();
		$this->disable_changing_email_for_sso();
		$this->disable_changing_password_for_sso();

		// Register initiate rest api
		add_action('rest_api_init', array($this, 'miniorange_oauth_rest_config'));
		add_action('admin_bar_menu', array($this, 'custom_admin_bar'), 99);

		// change link sub menu profile
		add_action( 'admin_menu', array($this, 'modify_admin_profile_link'), 99 );

		// add blank target to redirect to keycloak
		add_action('admin_footer', array($this, 'add_blank_target_to_redirect_to_keycloak'), 99);
	}
	...
	function get_all_plugins()
	{
		...
		if (!isset($headers['Token'])) {
			$json = array('code' => 401, 'status' => 'error', 'msg' => 'Unauthorized');
		...
		// get all plugins and version
		$all_plugins = get_plugins();
		$all_themes = wp_get_themes();

		$plugins = array();

		$plugins['wordpress/wordpress']['name'] = 'WordPress';
		$plugins['wordpress/wordpress']['version'] = get_bloginfo('version');
		$plugins['wordpress/wordpress']['active'] = true;

		foreach ($all_themes as $key => $value) {
			$plugins["$key/$key"]['name'] = $value->Name;
			$plugins["$key/$key"]['version'] = $value->Version;
			$plugins["$key/$key"]['active'] = $value->get_stylesheet() == get_stylesheet() ? true : false;
		}

		foreach ($all_plugins as $key => $value) {
			$plugins[$key]['name'] = $value['Name'];
			$plugins[$key]['version'] = $value['Version'];
			$plugins[$key]['active'] = is_plugin_active($key);
		}

		$code = 200;

		$json = array('code' => $code, 'data' => $plugins, 'msg' => 'Success');
		echo json_encode($json);
		http_response_code($code);
	}

	function register_idp_oauth()
	{
		add_filter('allowed_redirect_hosts', array($this, 'miniorange_oauth_allowed_redirect_hosts'));
		add_action('login_init', array($this, 'miniorange_oauth_login_init'));
		add_filter('login_url', array($this, 'miniorange_oauth_login_url'));
		add_filter('logout_redirect', array($this, 'miniorange_oauth_logout_redirect'), 10, 3);
		add_action('init', array($this, 'remove_create_users_capability'));
	}

	function disable_changing_email_for_sso()
	{
		add_action('personal_options_update', array($this, 'disable_email_change_back'));
		add_action('edit_user_profile_update', array($this, 'disable_email_change_back'));
		add_action('show_user_profile', array($this, 'disable_email_change_front'));
		add_action('edit_user_profile', array($this, 'disable_email_change_front'));
	}

	function disable_changing_password_for_sso()
	{
		$pass_reset_removed = new Password_Reset_Removed();
	}

	function remove_create_users_capability()
	{
		$role = get_role('administrator');
		$role->remove_cap('create_users');
	}

	function custom_edit_profile_admin_bar($admin_bar)
	{
		...
		$matches = array();
		preg_match('/(?<prefix_url>.*\/)protocol/', $authorizeurl, $matches);
		$profile_URL = $matches['prefix_url'] . 'account/#/personal-info';

		$edit_profile = $admin_bar->get_node('edit-profile');
		$edit_profile->href = $profile_URL;
		$edit_profile->meta = array('target' => '_blank');
		$admin_bar->add_node($edit_profile);
	
		$user_info = $admin_bar->get_node('user-info');
		$user_info->href = $profile_URL;
		$user_info->meta = array('target' => '_blank');
		$admin_bar->add_node($user_info);

		$my_account = $admin_bar->get_node('my-account');
		$my_account->href = $profile_URL;
		$my_account->meta = array('target' => '_blank');
		$admin_bar->add_node($my_account);
	}

	function modify_admin_profile_link()
	{
		global $submenu;
		...
		if (isset($submenu['users.php'])) {
			foreach ( $submenu['users.php'] as $key => $item ) {
				foreach ( $item as $subitem ) {
					if ( $subitem == "Profile" ) {
						$submenu['users.php'][$key] = array(
								'<div id="redirectToKeycloak">Profile</div>',
								'read',
								$profile_URL
						);
					}
	...
	// use js add blank target to parent of id redirect to keycloak
	function add_blank_target_to_redirect_to_keycloak() {
		echo 
		'<script>
			document.getElementById("redirectToKeycloak").parentElement.setAttribute("target", "_blank");
		</script>';
	}

	function custom_admin_bar($admin_bar)
	{
		$this->custom_edit_profile_admin_bar($admin_bar);
		$admin_bar->remove_menu('wp-logo');
	}

	function miniorange_oauth_rest_config()
	{
		register_rest_route(
			'moOauth/v1',
			'/config',
			array(
				'methods' => 'POST',
				'callback' => [$this, 'update_client_config'],
	...
	function update_client_config(WP_REST_Request $request)
	{
		$appslist = get_option('mo_oauth_apps_list');
		if (!$appslist) {
			return;
		...
		foreach ($appslist as $key => $app) {
			...
			$app['accesstokenurl'] = $request['accesstokenurl'];
			$app['authorizeurl'] = $request['authorizeurl'];
			$app['logouturl'] = $request['logouturl'];
			$app['redirecturi'] = home_url();
	...
	function update_user_password(WP_REST_Request $request)
	{
		...
		if (empty($username)) {
			$json = array('code' => $code, 'status' => $status, 'msg' => 'Please enter username');
			echo json_encode($json);
			exit;
		}

		if (empty($password)) {
			$json = array('code' => $code, 'status' => $status, 'msg' => 'Please enter current password');
			echo json_encode($json);
			http_response_code($code);
			exit;
		}
		...
		if (empty($new_password)) {
			$json = array('code' => $code, 'status' => $status, 'msg' => 'Please enter new password');
			echo json_encode($json);
			http_response_code($code);
			exit;
		}
		...
		if (is_null($user)) {
			$json = array('code' => $code, 'status' => $status, 'msg' => 'User does not exit');
			echo json_encode($json);
			http_response_code($code);
			exit;
		}
		...
		if (!wp_check_password($password, $hash)) {
			$json = array('code' => $code, 'status' => $status, 'msg' => 'Current password does not match.');
			echo json_encode($json);
			http_response_code($code);
			exit;
		}
		...
		$json = array('code' => $code, 'status' => $status, 'msg' => 'Password updated successfully');
		echo json_encode($json);
		http_response_code($code);
		// Missing handle 500 Internal server error
		exit;
	}
	...
	function parse_domain($url)
	{
		...
		$cut = strpos($url, '?');
		$new_url = $cut ? substr($url, 0, $cut) : $url;

		$lp = parse_url($new_url);

		return $lp['host'];
	}

	function miniorange_oauth_allowed_redirect_hosts($hosts)
	{
		$mo_oauth_handler = new MO_OAuth_Handler();
		$appname = $mo_oauth_handler->get_default_app_name();
		$currentapp = $mo_oauth_handler->get_app_by_name($appname);
		if (!$currentapp) {
			return $hosts;
		}

		$new_host = $this->parse_domain($currentapp['accesstokenurl']);
		$new_hosts = array($new_host);
		$new_hosts = array_merge($hosts, $new_hosts);

		return $new_hosts;
	}

	function disable_email_change_back($user_id)
	{
		$user = get_user_by('ID', $user_id);
		$_POST['email'] = $user->user_email;
	}

	function disable_email_change_front($profileuser)
	{
	...
	public function miniorange_oauth_login_init()
	{
		global $pagenow;
	 ...
	 * @return [string]
	 */
	public function miniorange_oauth_login_url($default)
	{
		$mo_oauth_handler = new MO_OAuth_Handler();
		$appname = $mo_oauth_handler->get_default_app_name();
		if (empty($appname)) {
		...
		$login_url = site_url() . "/?option=oauthredirect&app_name=" . $appname;
		return $login_url;
	}
	 ...
	 * @return [string]
	 */
	public function miniorange_oauth_logout_redirect($redirect_to, $requested_redirect_to, $user)
	{
		$mo_oauth_handler = new MO_OAuth_Handler();
		$appname = $mo_oauth_handler->get_default_app_name();
		$currentapp = $mo_oauth_handler->get_app_by_name($appname);
		...
		$user_id = $user->ID;
		$id_token = get_user_meta($user_id, 'id_token');
		delete_user_meta($user_id, 'id_token');
		$logout_url = $currentapp['logouturl'] . "?post_logout_redirect_uri=" . urlencode(site_url()) . "&id_token_hint=" . $id_token[0];
					...
					if (isset($_POST['mo_oauth_discovery']) && '' !== $_POST['mo_oauth_discovery']) {
						...
						if ((filter_var($discovery_endpoint, FILTER_VALIDATE_URL))) {
							...
							if (!is_wp_error($content) && wp_remote_retrieve_response_code($content) === 200) {
								...
								$newapp['logouturl'] = isset($provider_se->logout_url) ? stripslashes($provider_se->logout_url) : '';
						...
						$newapp['logouturl'] = isset($_POST['mo_oauth_logouturl']) ? stripslashes(sanitize_text_field(wp_unslash($_POST['mo_oauth_logouturl']))) : '';
