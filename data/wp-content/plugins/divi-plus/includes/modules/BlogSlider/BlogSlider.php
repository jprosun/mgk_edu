		...
		if ( $query->have_posts() ) {
			...
			if ( 'on' === $equalize_posts_height ) {
				...
				self::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl_blog_slider_post',
				...
				self::set_style(
					$render_slug,
					array(
						'selector'    => '%%order_class%% .dipl_blog_slider_read_more_link',
			...
			if ( 'on' === $show_arrow ) {
				...
				if ( ! empty( $arrows_position ) ) {
					wp_enqueue_script( 'dipl-blog-slider-custom', PLUGIN_PATH."includes/modules/BlogSlider/dipl-blog-slider-custom.min.js", array('jquery'), '1.0.1', true );
